...
import { IAdminDashboardStats, IPaginatedResponse, IUserOverview } from './admin.interface';
...
export class AdminService {
    static async getDashboardStats(): Promise<IAdminDashboardStats> {
        ...
        const totalRevenue = activeSubList.reduce((acc, sub) => acc + (sub.package.price || 0), 0);
        const totalStorageUsage = totalFiles.reduce((acc, file) => acc + file.size, BigInt(0));
    ...
    static async getAllUsers(
        ...
        const data: IUserOverview[] = users.map(user => {
            const usedStorage = user.files.reduce((acc, file) => acc + file.size, BigInt(0));
            return {
                id: user.id,
                name: user.name,
                email: user.email,
